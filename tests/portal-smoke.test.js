...
assert(/gwynne-park-run-club-v191/.test(serviceWorker), 'service worker cache should be bumped after today\'s deploy so returning browsers pick up the new build instead of serving stale cache-first assets');
...
assert(/styles\.css\?v=132/.test(homeHtml), 'home page should request the current contrast-safe stylesheet');
...
assert(/id="tab-carnival"/.test(adminDashboardHtml), 'coach hub should have a dedicated Carnival Day tab');
assert(/carnival-day-card/.test(adminDashboardHtml), 'carnival tab should include the Carnival Day panel');
assert(/carnival-create-form/.test(adminDashboardHtml), 'carnival day should include a create form');
assert(/carnival-type-select/.test(adminDashboardHtml), 'carnival create should choose athletics, cross country, or swimming');
assert(/carnival-program-list/.test(adminDashboardHtml), 'carnival day should render the event running order');
assert(/carnival-scoreboard/.test(adminDashboardHtml), 'carnival day should render a live faction scoreboard');
assert(/carnival-print-btn/.test(adminDashboardHtml), 'carnival day should offer a printable program');
assert(/carnival-division-setup-section/.test(adminDashboardHtml), 'carnival day should include a division setup section');
assert(/carnival-division-autosplit-btn/.test(adminDashboardHtml), 'carnival division setup should offer auto-split by time');
assert(/carnival-race-section/.test(adminDashboardHtml), 'carnival day should include a Record Race Results section');
assert(/carnival-race-year-select/.test(adminDashboardHtml), 'carnival race recorder should select a year group above the division');
assert(/carnival-mock-load-btn/.test(adminDashboardHtml), 'carnival day should offer a load-mock-carnival trial button');
assert(!/carnival-race-division-select[\s\S]{0,160}>Junior</.test(adminDashboardHtml), 'carnival race division select should no longer hardcode Junior/Intermediate/Senior age bands');
assert(/carnival-race-event-select/.test(adminDashboardHtml), 'carnival race recorder should pick events from the active carnival program');
assert(/carnival-race-division-select/.test(adminDashboardHtml), 'carnival race recorder should pick a Junior/Intermediate/Senior division');
assert(/carnival-race-roster/.test(adminDashboardHtml), 'carnival race recorder should list division runners with place selectors');
assert(/carnival-race-save-btn/.test(adminDashboardHtml), 'carnival race recorder should batch-save all placed runners in one action');
assert(/carnival-race-output/.test(adminDashboardHtml), 'carnival race recorder should show inline save status');
assert(/carnival-bonus-section/.test(adminDashboardHtml), 'carnival day should include a Bonus Points section');
assert(/carnival-bonus-house-wrap/.test(adminDashboardHtml), 'bonus points form should have a house/faction field (select when houseNames are configured)');
assert(/carnival-bonus-points/.test(adminDashboardHtml), 'bonus points form should have a points input (negatives allowed to dock)');
assert(/carnival-bonus-reason/.test(adminDashboardHtml), 'bonus points form should capture a custom reason label');
assert(/carnival-bonus-award-btn/.test(adminDashboardHtml), 'bonus points form should have an Award button');
assert(/carnival-bonus-ledger/.test(adminDashboardHtml), 'bonus points should render a running audit ledger');
...
assert(/id="carnival-minicoach-section"/.test(adminDashboardHtml), 'carnival day should include a Mini Coach checks section');
assert(/id="carnival-minicoach-notes"/.test(adminDashboardHtml), 'carnival Mini Coach should render advisory notes into a dedicated list container');
...
assert(/new-student-gender/.test(adminDashboardHtml), 'admin add-student form should optionally collect gender for carnival champions');
...
assert(/edit-student-gender/.test(adminDashboardHtml), 'admin student editor should optionally edit gender');
...
assert(/admin-dashboard\.js\?v=110/.test(adminDashboardHtml), 'admin dashboard should request the current live beta dashboard script');
...
assert(/headers\.indexOf\('house'\)/.test(adminDashboardJs) && /headers\.indexOf\('faction'\)/.test(adminDashboardJs), 'roster CSV import should accept an optional house or faction column');
assert(/headers\.indexOf\('gender'\)/.test(adminDashboardJs), 'roster CSV import should accept an optional gender column');
assert(/normalizeGenderValue/.test(adminDashboardJs), 'roster CSV import should normalize gender values (Boy/Girl/M/F) to boy/girl');
assert(/renderCarnivalChampions/.test(adminDashboardJs), 'carnival day should live-render gender-split division champions from carnival-day results');
assert(/house updated to/.test(adminDashboardJs), 'roster CSV import should backfill house on existing students instead of only skipping duplicates');
assert(/faction-settings-list/.test(adminDashboardHtml), 'settings should have an editable factions list');
assert(/id="faction-add-btn"/.test(adminDashboardHtml), 'settings should let you add a faction');
assert(/function factionNames/.test(adminDashboardJs), 'a factionNames() helper should source the configured factions');
assert(/populateFactionSelect/.test(adminDashboardJs), 'faction pickers should be populated from the configured list');
assert(/function carnivalPointsForField/.test(adminDashboardJs), 'carnival should compute field-size/tiered points via carnivalPointsForField');
assert(/CARNIVAL_EVENT_OPTIONS/.test(adminDashboardJs), 'carnival should use its own decoupled event pool, not the athletics division event options');
assert(/id="carnival-points-mode"/.test(adminDashboardHtml), 'carnival create form should let you choose field-size vs fixed points scoring');
...
assert(/CARNIVAL_KEY/.test(adminDashboardJs), 'admin dashboard should persist the active carnival');
assert(/SWIMMING_EVENT_OPTIONS/.test(adminDashboardJs), 'admin dashboard should define the swimming carnival event skeleton');
assert(/renderCarnivalDay/.test(adminDashboardJs), 'admin dashboard should render the carnival day panel');
assert(/carnivalPointsForPlace/.test(adminDashboardJs), 'carnival scoring should support a per-carnival points scheme');
assert(/\.filter\(Boolean\)\.map\(Number\)/.test(adminDashboardJs), 'parsePointsScheme must drop blank parts before Number() so an empty points input falls back to the school default instead of a bogus [0] scheme that scores every place 0');
assert(/crossCountryTeamScores/.test(adminDashboardJs), 'cross country carnivals should compute first-four team scores per house');
assert(/saveCarnivalRaceResults/.test(adminDashboardJs), 'carnival race recorder should batch-save a whole race of placings');
assert(/autoSplitDivisions/.test(adminDashboardJs), 'carnival should auto-split students into speed divisions by time');
assert(/Set up divisions for/.test(adminDashboardJs), 'carnival Mini Coach should nudge to set up divisions per event');
assert(/renderCarnivalRaceRoster/.test(adminDashboardJs), 'carnival race recorder should render the division roster with place selectors');
assert(/populateCarnivalRaceEvents/.test(adminDashboardJs), 'carnival race recorder should populate events from the active carnival program only');
...
assert(/renderCarnivalMiniCoach/.test(adminDashboardJs), 'carnival day should render a rule-based Mini Coach readiness checklist');
...
assert(/\.carnival-program/.test(styles), 'styles should include carnival program styles');
assert(/\.carnival-event--current/.test(styles), 'styles should highlight the up-next carnival event');
assert(/\.carnival-scoreboard/.test(styles), 'styles should include carnival scoreboard styles');
...
assert(/styles\.css\?v=132/.test(leaderboardHtml), 'leaderboard page should request the current stylesheet version');
assert(/styles\.css\?v=132/.test(interschoolTeamHtml), 'interschool team page should request the current stylesheet');
...
assert(/gwynne-park-run-club-v191/.test(serviceWorker), 'service worker cache should be bumped for the About page refresh');
