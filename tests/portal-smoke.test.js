...
assert(/gwynne-park-run-club-v199/.test(serviceWorker), 'service worker cache should be bumped after today\'s deploy so returning browsers pick up the new build instead of serving stale cache-first assets');
...
assert(/kiosk\.js\?v=2/.test(read('kiosk.html')), 'kiosk.html should cache-bust kiosk.js with a version pin so returning kiosks pick up scan-logic/audio changes');
assert(/bannerCopyFor/.test(kioskJs) && /First lap for/.test(kioskJs), 'kiosk should use context-aware banner copy (first-lap / round-lap variants, T18 shortlist)');
...
assert(/gwynne-park-run-club-v199/.test(serviceWorker), 'service worker cache should be bumped for the About page refresh');
