...
assert(/apply-branding-colors-btn/.test(adminDashboardHtml), 'branding settings should include an obvious button to apply school colours');
...
assert(/branding-logo-upload/.test(adminDashboardHtml), 'branding settings should collect an uploaded school emblem');
assert(/branding-file-input/.test(adminDashboardHtml), 'branding emblem upload should use a compact file input hit area');
assert(/branding-color-input/.test(adminDashboardHtml), 'branding colour pickers should use compact colour swatches');
...
assert(/branding-logo-preview/.test(adminDashboardHtml), 'branding settings should preview the active school emblem');
assert(/clear-branding-logo-btn/.test(adminDashboardHtml), 'branding settings should allow the uploaded school emblem to be cleared');
...
assert(/saveBrandingColorsOnly/.test(adminDashboardJs), 'admin dashboard should save school colours from the dedicated apply button');
assert(/previewBrandingColors/.test(adminDashboardJs), 'admin dashboard should preview school colours while editing');
assert(/applyBrandColorTokens/.test(adminDashboardJs), 'admin dashboard should apply school colour tokens consistently');
assert(/DEFAULT_SCHOOL_BLUE/.test(adminDashboardJs) && /DEFAULT_UNIFORM_GOLD/.test(adminDashboardJs), 'admin dashboard should keep explicit default school colour tokens');
...
assert(/brandingLogoUploadEl/.test(adminDashboardJs), 'admin dashboard should wire the school emblem upload control');
assert(/FileReader/.test(adminDashboardJs), 'admin dashboard should read uploaded school emblems as local data URLs');
assert(/logoDataUrl/.test(adminDashboardJs), 'admin dashboard should persist uploaded school emblems in branding settings');
assert(/clearBrandingLogo/.test(adminDashboardJs), 'admin dashboard should allow uploaded emblems to be cleared back to Corso');
...
assert(/styles\.css\?v=123/.test(leaderboardHtml), 'leaderboard page should request the current stylesheet version');
assert(/styles\.css\?v=123/.test(interschoolTeamHtml), 'interschool team page should request the current stylesheet');
...
assert(/logoDataUrl/.test(themeJs), 'shared theme script should apply uploaded school emblems');
assert(/schoolBlue/.test(themeJs) && /uniformGold/.test(themeJs), 'shared theme script should read coach-selected school colours');
assert(/applyBrandColors/.test(themeJs), 'shared theme script should apply coach-selected colour tokens');
assert(/setProperty\('--school-blue'/.test(themeJs), 'shared theme script should set the school blue CSS token from branding');
...
assert(/\.branding-color-input[\s\S]*width:\s*46px/.test(styles), 'School Settings colour picker should be a compact swatch');
assert(/\.branding-file-input[\s\S]*width:\s*max-content/.test(styles), 'School Settings file upload should only use the compact choose-file hit area');
