...
assert(/gwynne-park-run-club-v182/.test(serviceWorker), 'service worker cache should be bumped after today\'s deploy so returning browsers pick up the new build instead of serving stale cache-first assets');
...
assert(/admin-dashboard\.js\?v=102/.test(adminDashboardHtml), 'admin dashboard should request the current live beta dashboard script');
...
assert(/onboarding-house-names/.test(adminDashboardHtml), 'school settings should include a House / faction names setting');
assert(/House \/ faction names/.test(adminDashboardHtml), 'school settings should label the House / faction names input');
...
assert(/gwynne-park-run-club-v182/.test(serviceWorker), 'service worker cache should be bumped for the About page refresh');
